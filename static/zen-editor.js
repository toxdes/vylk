...
function nextInlineToken(text, start) {
  for (let index = start; index < text.length; index++) {
    ...
    const candidates = text.startsWith('***', index) ? [['***', 'zen-md-strong zen-md-em']]
      : text.startsWith('___', index) ? [['___', 'zen-md-strong zen-md-em']]
        : text.startsWith('**', index) ? [['**', 'zen-md-strong']]
          : text.startsWith('__', index) ? [['__', 'zen-md-strong']]
        : text.startsWith('~~', index) ? [['~~', 'zen-md-strike']]
          : text[index] === '*' ? [['*', 'zen-md-em']]
            : text[index] === '_' ? [['_', 'zen-md-em']]
              : text[index] === '`' ? [['`', 'zen-md-code']]
                : [];
...
class ZenEditor extends EventTarget {
  constructor(element) {
    ...
    element.addEventListener('compositionstart', () => {
      ...
      this.composingLine = selection?.focusNode && this.element.contains(selection.focusNode)
        ? (selection.focusNode.nodeType === Node.ELEMENT_NODE ? selection.focusNode : selection.focusNode.parentElement)?.closest?.('.zen-editor-line')
        : null;
    });
  ...
  get value() {
    if (this.cachedValue === null) this.cachedValue = this.lines.map(line => line.text).join('\n');
  ...
  selection() {
    const selection = global.getSelection?.();
    if (!selection?.rangeCount || !this.element.contains(selection.anchorNode) || !this.element.contains(selection.focusNode)) {
  ...
  pointOffset(node, offset) {
    const lineElement = (node.nodeType === Node.ELEMENT_NODE ? node : node.parentElement)?.closest?.('.zen-editor-line');
    const line = lineElement?.__zenLine;
    if (!line) return 0;
    const range = document.createRange();
    range.setStart(lineElement, 0);
    try { range.setEnd(node, offset); } catch (_) { range.setEnd(lineElement, lineElement.childNodes.length); }
  ...
  ensureSelectionVisible() {
    ...
    const lineElement = (selection.focusNode.nodeType === Node.ELEMENT_NODE ? selection.focusNode : selection.focusNode.parentElement)?.closest?.('.zen-editor-line');
    ...
    if (caretRect.top < editorRect.top + inset) this.element.scrollTop -= editorRect.top + inset - caretRect.top;
    else if (caretRect.bottom > editorRect.bottom - inset) this.element.scrollTop += caretRect.bottom - editorRect.bottom + inset;
  }
  ...
  textBetween(start, end) {
    ...
    if (first.line === last.line) return this.lines[first.line].text.slice(first.offset, last.offset);
    const parts = [this.lines[first.line].text.slice(first.offset)];
    for (let index = first.line + 1; index < last.line; index++) parts.push(this.lines[index].text);
  ...
  replaceRange(start, end, inserted, {inputType = 'insertText', record = true} = {}) {
    ...
    const combined = this.lines[first.line].text.slice(0, first.offset) + replacement + this.lines[last.line].text.slice(last.offset);
    ...
    if (first.line === last.line && nextTexts.length === 1) {
      ...
      const structuralStyleChanged = structuralLineKind(line.text) !== structuralLineKind(nextTexts[0]);
      const marker = line.element.querySelector('.zen-md-marker');
      const inlineStyleStarted = !marker && /[*_~`]/.test(nextTexts[0]) && Boolean(nextInlineToken(nextTexts[0], 0));
      if (!syntaxChanged && !structuralStyleChanged && !inlineStyleStarted && !marker && startPoint.node === endPoint.node && startPoint.node.nodeType === Node.TEXT_NODE) {
        ...
        this.lastSelection = {start:safeStart + replacement.length, end:safeStart + replacement.length, direction:'none'};
        if (record && (removed || replacement)) {
          this.history.push({start:safeStart, removed, inserted:replacement});
          if (this.history.length > 200) this.history.shift();
          this.future = [];
        }
        this.dispatchEvent(new CustomEvent('input', {detail:{inputType, data:replacement, length:this.length}}));
        return;
      }
      line.text = nextTexts[0];
    ...
    for (let index = first.line; index < this.lines.length; index++) this.lines[index].index = index;
    this.offsets = new FenwickTree(this.lines.map(line => line.text.length + 1));
    this.cachedValue = null;
    this.element.classList.toggle('is-empty', this.length === 0);
    const caret = safeStart + replacement.length;
  ...
  deleteBackward(selection, byWord = false) {
    if (selection.start !== selection.end) return this.replaceRange(selection.start, selection.end, '', {inputType:'deleteContentBackward'});
    ...
    this.replaceRange(start, selection.end, '', {inputType:byWord ? 'deleteWordBackward' : 'deleteContentBackward'});
  }

  deleteForward(selection, byWord = false) {
    if (selection.start !== selection.end) return this.replaceRange(selection.start, selection.end, '', {inputType:'deleteContentForward'});
    ...
    this.replaceRange(selection.start, end, '', {inputType:byWord ? 'deleteWordForward' : 'deleteContentForward'});
  }

  undo() {
    const transaction = this.history.pop();
    if (!transaction) return;
    this.replaceRange(transaction.start, transaction.start + transaction.inserted.length, transaction.removed, {inputType:'historyUndo', record:false});
  ...
  redo() {
    ...
    if (!transaction) return;
    this.replaceRange(transaction.start, transaction.start + transaction.removed.length, transaction.inserted, {inputType:'historyRedo', record:false});
  ...
  finishComposition() {
    ...
    renderLineContent(line.element, line.text);
    this.replaceRange(start, start + line.text.length, text, {inputType:'insertCompositionText'});
    this.setSelection(start + Math.min(text.length, caretInLine));
  }

  updateActiveLine() {
    const selection = global.getSelection?.();
    const lineElement = selection?.focusNode && this.element.contains(selection.focusNode)
      ? (selection.focusNode.nodeType === Node.ELEMENT_NODE ? selection.focusNode : selection.focusNode.parentElement)?.closest?.('.zen-editor-line')
      : null;
