(function () {
  ...
  const {
    APIError,
    errorFromPayload: apiErrorFromPayload,
    errorFromTransport: apiErrorFromTransport,
  ...
  function ensureZenSourceEditor() {
    if (!zenSourceEditor && zenSourceEditorElement && window.VylkZenEditor) {
      ...
      zenSourceEditor.addEventListener('input', (event) =>
        handleEditorSourceInput({
  ...
  function activateZenSourceEditor() {
    ...
    editor.setValue(
      editorSourceTextarea.value,
      editorSourceTextarea.selectionStart,
      editorSourceTextarea.selectionEnd,
    );
    editor.active = true;
  }
  ...
  const dashboardRouteState = routes.dashboardState;
  const noteRouteState = routes.noteState;
  const preferencesRouteState = routes.preferencesState;
  const isAppNoteRoute = routes.isNote;
  const isAppDashboardRoute = routes.isDashboard;
  const isAppPreferencesRoute = routes.isPreferences;
  const currentAppRouteState = routes.current;
  ...
  function setSyncDiagnostic(detail, responseStatus = 0) {
    lastSyncDiagnostic = String(detail || '')
      .replace(/\s+/g, ' ')
      .trim()
      .slice(0, 280);
  ...
  function cacheAppVersion(response) {
    const changedVersion =
      response?.version && appVersionAtLoad && response.version !== appVersionAtLoad;
    const changedRevision =
      response?.revision && appRevisionAtLoad && response.revision !== appRevisionAtLoad;
  ...
  async function fetchWithTimeout(
    path,
    options = {},
    {group = null, timeoutMs = httpRequestTimeoutMs} = {},
  ) {
  ...
  async function setIdleSyncStatus() {
    ...
    try {
      ...
      if (generation !== syncStatusGeneration || syncNetworkRequestsInFlight > 0 || syncInFlight)
        return;
  ...
  async function api(path, opts) {
    ...
    try {
      ...
      if (body === null)
        throw new APIError('invalid server response', {kind: 'protocol', retryable: false});
      return body;
    } catch (e) {
  ...
  async function syncFetch(path, options) {
    ...
    try {
      ...
      const data = response.status === 204 ? null : await responseBody(response);
      if (!response.ok)
        setSyncDiagnostic(
  ...
  function setConflictSelection(selection) {
    ...
    $('#conflict-selection-status').textContent =
      selection === 'local'
  ...
  function showConflictResolver(conflict) {
    ...
    $('#conflict-local-metadata').textContent =
      `Title: ${conflict.local.title || 'Untitled'}\nTags: ${conflict.local.tags || 'None'}`;
    $('#conflict-remote-metadata').textContent =
  ...
  $('#conflict-save').addEventListener('click', () => {
    void (activeConflictKind === 'remote-deleted'
      ? acceptDeletedConflict()
      : saveConflictResolution());
  });
  $('#conflict-copy').addEventListener('click', () => {
    void (activeConflictKind === 'remote-deleted'
      ? keepDeletedConflictAsCopy()
      : keepConflictAsCopy());
  });
  ...
  async function reconcileCompactedOperationLocally(operation, remote) {
    return withOfflineStore(['notes', 'queue'], 'readwrite', async (stores) => {
      ...
      for (const later of operations) {
        ...
        if (later.type !== 'note.save' && later.type !== 'note.delete' && later.type !== 'note.pin')
          continue;
        if (remote) {
      ...
      (stores) =>
        new Promise((resolve, reject) => {
          ...
          request.onsuccess = () => {
            ...
            const candidate = [
              ...batch.map((item) => outgoingSyncOperation(item)),
              outgoingSyncOperation(operation),
            ];
  ...
  async function applySyncAcknowledgement(operation, acknowledgement) {
    ...
    if (acknowledgement.status === 'conflict') {
      ...
      if (operation.type === 'note.pin') {
        ...
        if (!hasLaterSave)
          await rebaseQueuedNoteOperations(
            operation.note_id,
            operation.id,
            remote.revision,
            remote,
        ...
        const preserveLocalContent =
          hasLaterSave || (currentNoteId === operation.note_id && isDirty);
        const next = {
          ...remote,
    ...
    if (operation.type === 'note.save') {
      ...
      const queued = await pendingOperationsForNote(operation.note_id);
      const laterPin = latestLaterOperation(queued, operation, 'note.pin');
      const hasLater = await rebaseQueuedNoteOperations(
        operation.note_id,
        operation.id,
  ...
  async function flushPendingChanges() {
    ...
    for (;;) {
      ...
      const operations = await claimPendingOperationBatch(
        deviceID,
        maxOperations,
        syncPushBatchByteLimit,
      );
      ...
      const result = await syncFetch('/api/sync/push', {
        ...
        body: JSON.stringify({
          device_id: deviceID,
          operations: operations.map(outgoingSyncOperation),
        }),
      });
      pushed = true;
      ...
      if (!result.response.ok) {
        ...
        if (permanent) {
          ...
          if (
            await quarantineQueueOperation(
              operations[0],
  ...
  function mergeNestedPreferencePatch(current, base, desired) {
    ...
    new Set([...Object.keys(base), ...Object.keys(desired)]).forEach((key) => {
      ...
      if (
        !preferenceValuesEqual(currentValue, baseValue) &&
        !preferenceValuesEqual(currentValue, desiredValue)
      ) {
        conflict = true;
        return;
      }
  ...
  async function resolvePreferenceConflict(operation) {
    ...
    Object.entries(patch).forEach(([key, desired]) => {
      ...
      if (
        !(key in base) ||
        (!preferenceValuesEqual(current, base[key]) && !preferenceValuesEqual(current, desired))
      ) {
        conflicts.push(key);
  ...
  async function releaseSyncLease() {
    ...
    await withOfflineStore(['state'], 'readwrite', async (stores) => {
      const current = await requestValue(stores.state.get(syncLeaseKey));
      if (current?.value?.owner === syncTabID)
        await requestValue(stores.state.delete(syncLeaseKey));
    });
  }
  ...
  async function withSyncLeadership(work) {
    ...
    syncLeaseRenewTimer = setInterval(() => {
      void renewSyncLease();
    }, syncLeaseDurationMs / 3);
    try {
      return await work();
    } finally {
  ...
  function scheduleSync(options = {}, delayMs = 75) {
    ...
    syncScheduleTimer = setTimeout(async () => {
      ...
      if (syncFailed) {
        ...
        syncRetryDelayMs =
          syncRetryDelaysMs[index + 1] || syncRetryDelaysMs[syncRetryDelaysMs.length - 1];
  ...
  async function performSync(options = {}) {
    ...
    try {
      ...
      await globalThis.__vylkDependencies?.beforeSyncCompletion?.();
      ...
      if (!lastSyncDiagnostic)
        setSyncDiagnostic(error?.message || 'unknown sync error', error?.responseStatus);
  ...
  function isServerEventsHealthy() {
    return Boolean(
      serverEvents && serverHeartbeatAt > 0 && Date.now() - serverHeartbeatAt <= sseStaleAfterMs,
    );
  }
  ...
  function connectServerEvents() {
    ...
    events.addEventListener('server', (event) => {
      try {
        cacheAppVersion(JSON.parse(event.data));
      } catch (_) {}
    });
    events.addEventListener('change', (event) => {
      serverHeartbeatAt = Date.now();
      try {
        void handleServerChangeEvent(JSON.parse(event.data)).catch(() =>
          scheduleServerChangeSync(),
        );
      } catch (_) {
        scheduleServerChangeSync();
  ...
  function reconcileDashboardNotes(notes, conflicts) {
    ...
    if (notes.length === 0) {
      const message =
        dashboardHydrationState === 'loading'
          ? 'Loading notes…'
          : dashboardHydrationState === 'offline-empty'
    ...
    const existing = new Map(
      [...list.querySelectorAll(':scope > .note-item-row')].map((row) => [row.dataset.noteId, row]),
    );
  ...
  function renderDashboard(notes, conflicts) {
    ...
    let html =
      ...
      (currentTag ? '' : ' active') +
      '" data-tag="" aria-pressed="' +
      (currentTag ? 'false' : 'true') +
      '">All</button>';
    tags.forEach((t) => {
  ...
  async function openNote(id, {route = 'push'} = {}) {
    ...
    if (previewTimer) clearTimeout(previewTimer);
    if (
      !screens.editor.classList.contains('hidden') &&
      currentNoteId !== id &&
      (isDirty || localSavePromise)
    ) {
  ...
  async function restoreRoute({fetchRemote = false} = {}) {
    if (!isAppPreferencesRoute() && !$('#prefs-modal').classList.contains('hidden')) {
      ...
      if (
        isAppNoteRoute() &&
        !screens.editor.classList.contains('hidden') &&
        currentNoteId === history.state.noteID
      ) {
        if (isDirty) await saveCurrentNote(false);
        return;
      }
  ...
  function editorSnapshotIsCurrent(snapshot) {
    return (
      editorSessionGeneration === snapshot.sessionGeneration &&
  ...
  async function persistEditorSnapshot(snapshot) {
    ...
    const baseTitle = existing?.pending
      ? (existing.base_title ?? existing.title ?? '')
      : existing?.title || '';
    const baseTags = existing?.pending
    ...
    if (unresolved?.kind === 'remote-deleted') {
      ...
      if (editorSnapshotIsCurrent(snapshot)) {
        ...
        isDirty = false;
        if (!restoringHistoryRoute && noteIDFromLocation() !== snapshot.noteID)
    ...
    try {
      const persistLocalNote =
        globalThis.__vylkDependencies?.saveLocalNoteAndQueue || saveLocalNoteAndQueue;
  ...
  function showTablePicker(trigger) {
    ...
    const left = Math.min(
      ...
      window.innerWidth - tablePicker.offsetWidth - gutter,
    );
    const top = Math.min(
      rect.bottom + gutter,
      window.innerHeight - tablePicker.offsetHeight - gutter,
    );
  ...
  function insertTable(rows, columns) {
    ...
    const result = window.VylkMarkdownFormatting.table(
      ta.value,
      ta.selectionStart,
      ta.selectionEnd,
      rows,
    ...
    if (zenSourceIsActive()) zenSourceEditor.setValue(result.value, result.cursor, result.cursor);
  ...
  document.addEventListener('pointerdown', (e) => {
    if (
      ...
      !e.target.closest('#table-picker, [data-fmt="table"]')
    )
      hideTablePicker();
  });
  ...
  function insertFmt(type) {
    ...
    const result = window.VylkMarkdownFormatting.format(
      ta.value,
      ta.selectionStart,
      ta.selectionEnd,
      type,
  ...
  function setPanelState(state) {
    ...
    document
      ...
      .forEach((button) => {
        button.setAttribute(
          'aria-pressed',
          String(state === 'zen' && button.dataset.zenAction === zenViewState),
        );
      });
    ...
    if (visibleState !== 'editor') {
      if (
        ...
        editorSourceValue() !== renderedPreviewSource
      ) {
        requestPreviewRender({announceBusy: true});
  ...
  function applyPanelRatio() {
    $('#editor-panels').style.setProperty(
      '--editor-panel-width',
  ...
  function syncInteractivePreviewUI() {
    ...
    textarea.setAttribute(
      'aria-label',
      interactiveSourceLocked ? 'Markdown source, read-only during drag' : 'Markdown note content',
    );
  ...
  function undecoratePreviewEntry(entry, options) {
    if (
      !entry ||
      (activePreviewDrag?.sourceStart === entry.start && activePreviewDrag?.scope === entry.scope)
    )
      return;
  ...
  function decorateInteractivePreview() {
    ...
    if (typeof IntersectionObserver !== 'function') {
      if (entries.length <= 100) entries.forEach(decoratePreviewEntry);
        ...
        const process = () => {
          ...
          while (index < entries.length && performance.now() - started < 5)
            decoratePreviewEntry(entries[index++]);
    ...
    disconnectPreviewDecorationObserver();
    const generation = previewDecorationGeneration;
    previewDecorationObserver = new IntersectionObserver(
      (records) => {
        const entering = [];
        const leaving = [];
    ...
    const nextObservedElements = new Set(
      entries.map(previewEntryObservationElement).filter(Boolean),
    );
    const pendingEntries = entries.filter((entry) =>
      nextObservedElements.has(previewEntryObservationElement(entry)),
    );
    let index = 0;
    const observe = () => {
      previewDecorationHandle = null;
  ...
  function isPreviewVisible() {
    return (
      ...
      (panelState === 'both' ||
        panelState === 'preview' ||
        (panelState === 'zen' && zenViewState === 'preview'))
    );
  }
  ...
  function calculatePreviewScrollAdjustment({
    previewTop,
    previewHeight,
    previewScrollTop,
    previewScrollHeight,
  ...
  function measureEditorCaret() {
    ...
    if (source.length > 250_000) {
      ...
      const contentHeight = Math.max(
        lineHeight,
        ta.scrollHeight - paddingTop - (parseFloat(computed.paddingBottom) || 0),
      );
      const offsetTop =
        paddingTop +
        (source.length
    ...
    if (
      editorCaretMeasurementCache?.source === source &&
      editorCaretMeasurementCache.position === position &&
      editorCaretMeasurementCache.metricsKey === metricsKey
    ) {
      return {
    ...
    if (
      !editorCaretMirror ||
      !editorCaretRange ||
      (!editorCaretMirrorText && !editorCaretMarker) ||
  ...
  function updateEditorCaretCue() {
    ...
    const focused =
      panelState !== 'zen' && document.activeElement === textarea && !textarea.readOnly;
  ...
  function centerEditorCaretInView(targetRatio = 0.38, {defer = true} = {}) {
    const textarea = editorSourceTextarea;
    if (!textarea) return;
    const center = () => {
      const caret = measureEditorCaret();
  ...
  function centerZenCaretNow({defer = true} = {}) {
    ...
    const center = () => {
      ...
      if (panelState === 'zen' && document.activeElement === editorSourceTextarea) {
        centerEditorCaretInView(0.48, {defer: false});
      }
    };
    if (!defer) {
      cancelScheduledZenCaretCenter();
  ...
  function previewTaskCheckbox(item) {
    ...
    return (
      item.querySelector(':scope > input[type="checkbox"], :scope > p > input[type="checkbox"]') ||
  ...
  function alignPreviewWithCaret(block, range) {
    ...
    const sourceProgress = Math.min(
      1,
      Math.max(0, (editorSourceSelection().start - range.start) / sourceLength),
    );
    const anchorTop = blockRect.top + blockRect.height * sourceProgress;
    const caretTop = caret.top + caret.height / 2;
    const margin = Math.max(caret.lineHeight * 2, Math.min(96, preview.clientHeight * 0.18));
  ...
  function previewBlockDescriptors(source, renderMetadata) {
    if (renderMetadata?.source === source && Array.isArray(renderMetadata.blocks))
      return renderMetadata.blocks;
    const descriptors = [];
    let offset = 0;
    for (const token of marked.lexer(source, markdownRenderOptions())) {
      ...
      descriptors.push({
        ...
        type: token.type,
        listItems:
          token.type === 'list' && window.VylkInteractive?.listItemRanges
  ...
  function previewContentBlocks(pv, patchedBlocks) {
    ...
    return Array.from(pv.children)
      .filter(
        (c) =>
          c.tagName && !['STYLE', 'SCRIPT'].includes(c.tagName) && !c.dataset.previewDragIndicator,
      )
      .map((element) =>
  ...
  function createPreviewCacheState(renderMetadata, patchedBlocks) {
    ...
    const previousBlockEntries = new Map(
      interactiveBlockItems.map((entry) => [entry.contentElement || entry.element, entry]),
    );
    const previousListEntries = new Map(
      interactiveListItems.map((entry) => [entry.element, entry]),
    );
  ...
  function commitPreviewCache(state, generation) {
    if (
      ...
      state.source !== editorSourceValue() ||
      state.blocks !== previewBlocks
    )
      return;
  ...
  function cachePreviewBlocks(renderMetadata = null, patchedBlocks = null, {defer = false} = {}) {
    ...
    const process = () => {
      ...
      try {
        while (
          state.index < state.descriptors.length &&
          (!defer || performance.now() - started < 5)
        )
          processPreviewCacheBlock(state);
      } catch (_) {
        previewBlockRanges = [];
        return;
      }
  ...
  function highlightBlock() {
    ...
    let highlightTarget = interactivePreviewActive
      ? block.closest('.interactive-preview-card') || block
      : block;
    if (['UL', 'OL'].includes(block.tagName)) {
      ...
      if (listItem?.element?.isConnected) {
        ...
        range = listItem;
        highlightTarget = interactivePreviewActive
          ? listItem.card || listItem.element
          : listItem.element;
      }
    }
  ...
  const previewAllowedAttributes = new Set([
    'align',
    'alt',
    'checked',
    'class',
  ...
  function sanitizePreview(container) {
    [...container.querySelectorAll('*')].forEach((element) => {
      ...
      attributeNames.forEach((attributeName) => {
        ...
        if (!previewAllowedAttributes.has(name) || name.startsWith('on'))
  ...
  document.addEventListener('pointerdown', (event) => {
    if (!interactivePreviewActive || event.target.closest('#preview .interactive-preview-card'))
      return;
  ...
  function cancelPreviewDrag({animateReturn = true} = {}) {
    ...
    const returnLayout =
      drag.armed && animateReturn
        ? capturePreviewLayout(drag.placeholder?.parentNode, drag.sourceElement)
        : null;
    if (drag.armed) {
  ...
  function armPreviewDrag(drag = activePreviewDrag) {
    ...
    if (
      !interactivePreviewSourceIsCurrent() ||
      !drag.sourceElement?.isConnected ||
      !drag.visualElement?.isConnected
    ) {
      cancelPreviewDrag({animateReturn: false});
      return false;
    }
  ...
  function finishPreviewDrag(event) {
    ...
    if (event.type === 'pointerup' && drag.armed && drag.target) {
      ...
      const source = editorSourceValue();
      const entries = [...interactiveBlockItems, ...interactiveListItems];
      const change = window.VylkInteractive?.moveMarkdownUnit(
        source,
        entries,
        drag.sourceStart,
        drag.scope,
      ...
      if ($('#preview').hasPointerCapture?.(event.pointerId))
        $('#preview').releasePointerCapture(event.pointerId);
      if (change)
        applyInteractiveSource(change.source, {
          start: change.start,
          removed: source.slice(change.start, change.end),
          inserted: change.inserted,
        });
  ...
  function renderPreviewBlocksProgressively(md, renderMetadata) {
    ...
    const process = () => {
      ...
      let batchSize = 0;
      while (
        index < renderMetadata.blocks.length &&
        batchSize < 12 &&
        performance.now() - started < 5
      ) {
        const element = previewElementFromHTML(renderMetadata.blocks[index++]);
        if (!element) {
          preview.removeAttribute('aria-busy');
          renderPreviewHTML(md, renderMetadata.blocks.map((block) => block.html || '').join(''), {
            ...renderMetadata,
  ...
  function patchPreviewBlocks(renderMetadata) {
    ...
    if (
      ...
      !renderMetadata?.incrementalSafe ||
      previous.source !== renderedPreviewSource
    )
      return null;
    if (
      !previous.blocks?.every((block) => typeof block.html === 'string') ||
      !renderMetadata.blocks?.every((block) => typeof block.html === 'string')
    )
      return null;
    ...
    while (
      ...
      prefix < renderMetadata.blocks.length &&
      previous.blocks[prefix].tagName === renderMetadata.blocks[prefix].tagName &&
      previous.blocks[prefix].html === renderMetadata.blocks[prefix].html
    )
      prefix++;
    let suffix = 0;
    while (
      suffix < current.length - prefix &&
      suffix < renderMetadata.blocks.length - prefix &&
      previous.blocks[previous.blocks.length - suffix - 1].tagName ===
        renderMetadata.blocks[renderMetadata.blocks.length - suffix - 1].tagName &&
      previous.blocks[previous.blocks.length - suffix - 1].html ===
        renderMetadata.blocks[renderMetadata.blocks.length - suffix - 1].html
    )
      suffix++;
    ...
    if (
      ...
      currentMiddle.every((element, index) => {
  ...
  function renderPreviewHTML(md, html, renderMetadata = null) {
    ...
    if (
      patchedBlocks === null &&
      renderMetadata?.incrementalSafe &&
      renderMetadata.blocks.length > 80
    ) {
    ...
    if (patchedBlocks === null) {
      ...
      fragment.innerHTML =
        typeof html === 'string'
          ? html
          : renderMetadata?.blocks?.map((block) => block.html || '').join('') || '';
  ...
  function ensurePreviewRenderWorker() {
    if (previewRenderWorker || previewWorkerUnavailable || typeof Worker !== 'function')
      return previewRenderWorker;
    try {
      previewRenderWorker = new Worker('/js/workers/preview-worker.js');
  ...
  function normalizeShortcutPrefix(value) {
    ...
    if (!window.VylkShortcuts?.isAllowedPrefix(binding))
      return {steps: [{key: '/', modifiers: ['Mod']}]};
    return binding;
  }
  ...
  function normalizePrefs(value = {}, fallback = {}) {
    ...
    if (!['normal', 'compact', 'off'].includes(merged.statusDisplay))
      merged.statusDisplay = DEFAULT_PREFS.statusDisplay;
    if (!CONTENT_WIDTH_VALUES.includes(merged.contentWidth))
      merged.contentWidth = DEFAULT_PREFS.contentWidth;
    if (!CONTENT_WIDTH_VALUES.includes(merged.zenPageWidth))
      merged.zenPageWidth = DEFAULT_PREFS.zenPageWidth;
    ...
    if (['editor', 'preview', 'split', 'zen'].includes(savedStartView))
      ...
      merged.startView =
        (value.hidePreview ?? fallback.hidePreview) ? 'editor' : DEFAULT_PREFS.startView;
    if (!['panel', 'header'].includes(merged.saveButtonLocation))
      merged.saveButtonLocation = DEFAULT_PREFS.saveButtonLocation;
  ...
  function applyTheme(themeID = prefs.theme) {
    ...
    Object.entries(theme.vars).forEach(([name, value]) =>
      root.style.setProperty(`--${kebabCase(name)}`, value),
    );
  ...
  async function applyFontsNow(clearCache = false) {
    ...
    const families = [
      ...new Set(
        FONT_SLOTS.filter(shouldFetchGoogleFont)
    ...
    await Promise.all(
      families.map(async (fontFamily) => {
        ...
        const loadedStylesheet = waitForStylesheet(link, 5000);
  ...
  function renderThemeOptions() {
    ...
    select.innerHTML = themeDefinitions
  ...
  function renderFontOptions() {
    FONT_SLOTS.forEach((slot) => {
      ...
      if (sizeSelect) {
        sizeSelect.innerHTML = FONT_SIZE_OPTIONS.map(
  ...
  function openPreferences({route = 'push'} = {}) {
    ...
    $('#pref-accent').value =
      prefs.accentColor || themeByID.get(prefs.theme)?.vars.accent || '#ae2448';
  ...
  const preferenceSaveTasks = new Set();

  function savePref(key, value) {
    const task = persistPreference(key, value);
    preferenceSaveTasks.add(task);
    void task.then(
      () => preferenceSaveTasks.delete(task),
      () => preferenceSaveTasks.delete(task),
    );
    return task;
  }
  ...
  $('#pref-start-view').addEventListener('change', function () {
    void savePref('startView', this.value);
  });
  ...
  $('#pref-save-location').addEventListener('change', function () {
    void savePref('saveButtonLocation', this.value);
  });
  ...
  $('#pref-zen-word-count').addEventListener('change', function () {
    void savePref('zenWordCount', this.checked);
  });
  $('#pref-zen-show-title').addEventListener('change', function () {
    void savePref('zenShowTitle', this.checked);
  });
  $('#pref-zen-show-controls').addEventListener('change', function () {
    void savePref('zenShowControls', this.checked);
  });
  $('#pref-theme').addEventListener('change', function () {
    void savePref('theme', this.value);
  });
  $('#pref-accent').addEventListener('change', function () {
    void savePref('accentColor', this.value);
  });
  ...
  FONT_SLOTS.forEach((slot) => {
    ...
    input.addEventListener('input', function () {
      if (!$(slot.error)?.hidden)
        setFontError(
          slot,
          validFontValue(this.value)
            ? ''
  ...
  $('#pref-status').addEventListener('change', function () {
    void savePref('statusDisplay', this.value);
  });
  $('#pref-content-width').addEventListener('change', function () {
    void savePref('contentWidth', this.value);
  });
  $('#pref-zen-page-width').addEventListener('change', function () {
    void savePref('zenPageWidth', this.value);
  });
  ...
  function shortcutBindingFor(command) {
    const binding = Object.hasOwn(prefs.keyboardShortcuts, command.id)
      ? prefs.keyboardShortcuts[command.id]
  ...
  function commandForBinding(binding, {sequence = false} = {}) {
    return shortcutCommands.find((command) => {
      ...
      return (
        ...
        window.VylkShortcuts.sameBinding(current, binding)
      );
    });
  }
  ...
  function renderShortcutPreferences() {
    ...
    if (prefix) {
      ...
      prefix.textContent = recording
        ? 'Press prefix…'
        : window.VylkShortcuts.displayBinding(shortcutPrefixBinding());
      prefix.setAttribute(
        'aria-label',
        recording
          ? 'Recording shortcut prefix'
          : `Record shortcut prefix, currently ${window.VylkShortcuts.ariaBinding(shortcutPrefixBinding())}`,
      );
    }
    ...
    byGroup.forEach((commands, group) => {
      ...
      commands.forEach((command) => {
        ...
        const binding = shortcutBindingFor(command);
        const recording =
          activeShortcutRecording?.type === 'command' && activeShortcutRecording.id === command.id;
        record.textContent = recording
          ? activeShortcutRecording.awaitingSequenceKey
            ? 'Prefix,'
            : 'Press shortcut…'
          : shortcutPreferenceLabel(binding);
        record.setAttribute(
          'aria-label',
  ...
  function shortcutBindingConflict(commandID, binding) {
    return shortcutCommands.find(
      (command) =>
        ...
        window.VylkShortcuts.sameBinding(shortcutBindingFor(command), binding),
    );
  }
  ...
  async function setShortcutOverride(commandID, binding) {
    ...
    if (binding && defaultBinding && window.VylkShortcuts.sameBinding(binding, defaultBinding))
  ...
  function startShortcutSequence() {
    ...
    const available = shortcutCommands.filter(
      (item) => commandCanRun(item) && shortcutBindingFor(item)?.steps.length === 2,
    );
  ...
  document.addEventListener(
    ...
    (event) => {
      if (activeShortcutRecording) {
        ...
        if (
          activeShortcutRecording.type === 'command' &&
      ...
      if (window.VylkShortcuts.isLeader(first, shortcutPrefixBinding().steps[0])) {
        const hasSequence = shortcutCommands.some(
          (command) => commandCanRun(command) && shortcutBindingFor(command)?.steps.length === 2,
        );
