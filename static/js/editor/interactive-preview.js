(function () {
  ...
  function reorderListItems(source, entries, movingStart, targetStart, placement = 'before') {
    ...
    const chunks = siblings.map((entry, index) =>
      source.slice(entry.start, index + 1 < siblings.length ? siblings[index + 1].start : end),
    );
  ...
  function moveMarkdownUnit(
    ...
    const moving = entries.find(
      (entry) => entry.start === movingStart && entry.scope === movingScope,
    );
    const target = entries.find(
      (entry) => entry.start === targetStart && entry.scope === targetScope,
    );
    if (!moving || !target || moving === target) return null;
    ...
    const inserted = markdownJoin(
      markdownJoin(withoutMoving.slice(0, markerIndex), chunk),
      withoutMoving.slice(markerIndex + markerLength),
    );
    const normalized =
