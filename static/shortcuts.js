...
function normalizeModifiers(value) {
  ...
  const normalized = [...new Set(modifiers.filter(modifier => modifier === 'Mod' || modifier === 'Shift'))];
...
function normalizeBinding(value) {
  const steps = Array.isArray(value?.steps) ? value.steps.map((step, index) => normalizeStep(step, {allowBare: index === 1})) : [];
...
function sameBinding(left, right) {
  ...
  return Boolean(a && b && a.steps.length === b.steps.length && a.steps.every((step, index) => sameStep(step, b.steps[index])));
}
...
function eventMatchesStep(event, value) {
  ...
  return Boolean(primaryPressed) === step.modifiers.includes(PRIMARY)
    && !otherPrimaryPressed
...
function isReservedBinding(value) {
  ...
  return step.modifiers.includes('Shift') ? reservedShiftKeys.has(step.key) : reservedDirectKeys.has(step.key);
}

function isAllowedPrefix(value) {
  const binding = normalizeBinding(value);
  return Boolean(binding && binding.steps.length === 1 && binding.steps[0].modifiers.includes(PRIMARY));
}
