...
var errRevisionConflict = store.ErrRevisionConflict
...
func (a *app) handleListNotes(w http.ResponseWriter, r *http.Request) {
	...
	if limitParam != "" {
		...
		page, err := store.ListNotesPage(a.db, tag, r.URL.Query().Get("cursor"), limit)
		if err != nil {
	...
	notes, err := store.ListNotes(a.db, tag)
	if err != nil {
		httpx.WriteAPIError(w, http.StatusInternalServerError, "list_notes_failed", "could not list notes")
		return
	}
...
func (a *app) handleSearchNotes(w http.ResponseWriter, r *http.Request) {
	...
	notes, err := store.SearchNotes(a.db, query, 50)
	if err != nil {
...
func (a *app) handleSyncChanges(w http.ResponseWriter, r *http.Request) {
	...
	page, err := store.ListSyncChanges(a.db, since, limit)
	if err != nil {
...
func currentNoteRevision(db *sql.DB, id string) (int64, error) {
	return store.CurrentNoteRevision(db, id)
}
...
func (a *app) handleSaveNote(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Title) > notepkg.MaxTitleBytes || len(req.Tags) > notepkg.MaxTagsBytes {
	...
	tags := notepkg.NormalizeTags(req.Tags)
...
func (a *app) handleSavePrefs(w http.ResponseWriter, r *http.Request) {
	...
	if err := store.SavePrefs(a.db, &p, expectedRevision); errors.Is(err, errRevisionConflict) {
		current, currentErr := store.GetPrefs(a.db)
		if currentErr != nil {
			httpx.WriteAPIError(w, http.StatusInternalServerError, "load_preferences_failed", "could not load preferences")
			return
		}
